Feature: SMR - Create SMR - Link Assets - Fixed Schedule - convert to Work Order

  @smr @smrFixedSchedule @sanitySMR @e2e
  Scenario: Create a new Scheduled Maintenance Record with Fixed Schedule
    Given the user logs into the application
    And the user selects a specific database
    When the user navigates to the Scheduled Maintenance Records page
    And the user creates a new SMR "Fixed Schedule" with a unique description and all mandatory fields
